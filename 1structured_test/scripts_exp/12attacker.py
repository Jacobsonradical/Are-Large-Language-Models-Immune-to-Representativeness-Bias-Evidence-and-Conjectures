...
if __name__ == "__main__":
    exp_amb_dir = "../1raw/12attacker"
    os.makedirs(exp_amb_dir, exist_ok=True)
    with open("12attacker_begin.txt", "r") as f:  # CHANGE !!!
        exp_begin = f.read()
    with open("12attacker_choice.json", "r") as f:  # CHANGE !!!
        exp_choice = json.load(f)
    with open("12attacker_end.txt", "r") as f:  # CHANGE !!!
        exp_end = f.read()

    model_round = {"gpt-4o": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gpt-4-turbo": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gpt-3.5-turbo-0125": [(i, i + 9) for i in range(1, 381, 9 + 1)],
                   "gemini-1.5-pro-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gemini-1.5-flash-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gemini-1.0-pro-latest": [(i, i + 9) for i in range(1, 571, 9 + 1)],
                   ...
                   "mistral-small-latest": [(i, i + 9) for i in range(1, 241, 9 + 1)]}
    ...
    with open("12attacker_end-special.txt", "r") as f:  # CHANGE !!!
        exp_end_2 = f.read()
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures_2 = [
