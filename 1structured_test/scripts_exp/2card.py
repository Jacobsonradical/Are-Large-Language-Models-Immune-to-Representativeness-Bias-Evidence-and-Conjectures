...
if __name__ == "__main__":
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures = [
            ...
            for start, end in [(i, i + 3) for i in range(1, 151, 4)]
        ]

    # Mistral-Large needs special design of prompt to return a parse-able answer.
    with open("2card_mistral-large.txt", "rt") as f:
        exp_prompt_mistral_large = f.read()
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures = [
            executor.submit(exp, exp_amb_dir, exp_prompt_mistral_large, "mistral-large-latest", start, end, 2000)
            for start, end in [(i, i + 3) for i in range(1, 151, 4)]
        ]
