...
if __name__ == "__main__":
    exp_amb_dir = "../1raw/10attacker"
    os.makedirs(exp_amb_dir, exist_ok=True)
    exp_choice = ["85% of the population in the city is shorter than 185 cm, and 15% are 185 cm or taller.",
                  "There were two witnesses to the accident. One claimed that the attacker had been 185 cm or taller, "
                  "and the other claimed that the attacker had been shorter than 185 cm. The court assessed the "
                  "witness's ability to judge a person's height under conditions similar to that night. Given a "
                  "sample of people, half shorter than 185 cm and half 185 cm or taller, the first witness was able "
                  "to identify the correct height about 80% of the time"
                  "and erred in 20% of the cases. The second witness was able to identify the correct height about "
                  "70% of the time and erred in 30% of the cases."]
    exp_begin = ("A person was attacked from behind at night and the victim could not see the attacker's height. "
                 "Despite this, we have the following two available information:")
    exp_end = ("What is your estimate of the probability that the attacker's height was 185 cm or taller, "
               "as the first witness claimed?"
               "\n\nPlease answer the question in the following format: "
               "{probability: your estimate, please provide a number between 0 and 1}")

    # open-mistral-7b excluded due to poor performance and high number of sample required.
    model_round = {"gpt-4o": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "gpt-4-turbo": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "gpt-3.5-turbo-0125": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "gemini-1.5-pro-latest": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "gemini-1.5-flash-latest": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "gemini-1.0-pro-latest": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "claude-3-opus-20240229": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "claude-3-sonnet-20240229": [(i, i + 9) for i in range(1, 331, 9 + 1)],
                   "claude-3-haiku-20240307": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   # "open-mistral-7b": [(i, i + 9) for i in range(1, 801, 9 + 1)],
                   "mistral-large-latest": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "mistral-medium-latest": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   "mistral-small-latest": [(i, i + 9) for i in range(1, 301, 9 + 1)]}
    ...
    #  open-mixtral-8x22b and open-mixtral-8x7b needs special design of end_prompt for parsable answer.
    model_round_2 = {"open-mixtral-8x22b": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                     "open-mixtral-8x7b": [(i, i + 9) for i in range(1, 681, 9 + 1)]}
    exp_end_2 = ("What is your estimate of the probability that the attacker's height was 185 cm or taller, "
                 "as the first witness claimed?"
                 "\n\nFirst, feel free to reason and compute. And then "
                 "please answer the question strictly in the following format:"
                 "{probability: your estimate, please provide a number between 0 and 1}")
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures_2 = [
