...
if __name__ == "__main__":
    exp_amb_dir = "../1raw/9attacker"
    os.makedirs(exp_amb_dir, exist_ok=True)
    with open("9attacker.txt", "rt") as f:
        exp_prompt = f.read()
    ...
    model_round = {"gpt-4o": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                   ...
                   "claude-3-sonnet-20240229": [(i, i + 9) for i in range(1, 301, 9 + 1)],
    ...
    # open-mixtral-8x7b and open-mixtral-8x22b needs special design of end_prompt for parsable answer.
    with open("9attacker_special.txt", "rt") as f:
        exp_prompt_2 = f.read()
    model_round_2 = {"open-mixtral-8x22b": [(i, i + 9) for i in range(1, 301, 9 + 1)],
                     "open-mixtral-8x7b": [(i, i + 9) for i in range(1, 301, 9 + 1)]}
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures_2 = [
            executor.submit(exp, exp_amb_dir, exp_prompt_2, exp_model_name_2, start, end, 2000)
