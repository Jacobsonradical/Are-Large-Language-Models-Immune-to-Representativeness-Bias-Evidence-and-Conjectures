...
if __name__ == "__main__":
    exp_amb_dir = "../1raw/13dreamer"
    os.makedirs(exp_amb_dir, exist_ok=True)
    with open("13dreamer.txt", "rt") as f:
        prompt_q8_1 = f.read()  # CHANGE !!!
    # mistral-medium-latest and open-mistral-7b are excluded because they could not follow instruction (tried
    # several) to give parseable answer
    model_round = {"gpt-4o": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gpt-4-turbo": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gpt-3.5-turbo-0125": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   ...
                   "open-mixtral-8x22b": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "open-mixtral-8x7b": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   # "open-mistral-7b": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "mistral-large-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   # "mistral-medium-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "mistral-small-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)]}
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures_q8_1 = [
            executor.submit(exp, exp_amb_dir, prompt_q8_1, exp_model_name, start, end, 2000)
            for exp_model_name, ranges in model_round.items()
            for start, end in ranges
        ]

    exp_amb_dir = "../1raw/14dreamer"
    os.makedirs(exp_amb_dir, exist_ok=True)
    with open("14dreamer.txt", "rt") as f:
        prompt_q8_2 = f.read()
    # mistral-medium-latest and open-mistral-7b are excluded because they could not follow instruction (tried
    # several) to give parseable answer
    model_round = {"gpt-4o": [(i, i + 9) for i in range(1, 151, 9 + 1)],
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures_q8_2 = [
            executor.submit(exp, exp_amb_dir, prompt_q8_2, exp_model_name, start, end, 2000)
            for exp_model_name, ranges in model_round.items()
            for start, end in ranges
        ]

    exp_amb_dir = "../1raw/15dreamer"
    os.makedirs(exp_amb_dir, exist_ok=True)
    with open("15dreamer.txt", "rt") as f:
        prompt_q8_3 = f.read()
    # mistral-medium-latest and open-mistral-7b are excluded because they could not follow instruction (tried
    # several) to give parseable answer
    model_round = {"gpt-4o": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gpt-4-turbo": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gpt-3.5-turbo-0125": [(i, i + 9) for i in range(1, 191, 9 + 1)],
                   "gemini-1.5-pro-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gemini-1.5-flash-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "gemini-1.0-pro-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)],
                   "claude-3-opus-20240229": [(i, i + 9) for i in range(1, 151, 9 + 1)],
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures_q8_3 = [
            executor.submit(exp, exp_amb_dir, prompt_q8_3, exp_model_name, start, end, 2000)
            for exp_model_name, ranges in model_round.items()
            for start, end in ranges
        ]

    exp_amb_dir = "../1raw/16dreamer"
    os.makedirs(exp_amb_dir, exist_ok=True)
    with open("16dreamer.txt", "rt") as f:
        prompt_q8_4 = f.read()
    # mistral-medium-latest and open-mistral-7b are excluded because they could not follow instruction (tried
    # several) to give parseable answer
    model_round = {"gpt-4o": [(i, i + 9) for i in range(1, 151, 9 + 1)],
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures_q8_4 = [
            executor.submit(exp, exp_amb_dir, prompt_q8_4, exp_model_name, start, end, 2000)
            for exp_model_name, ranges in model_round.items()
            for start, end in ranges
        ]

    # mistral-large-latest needs special prompt for parsable answer.
    exp_amb_dir = "../1raw/16dreamer"
    os.makedirs(exp_amb_dir, exist_ok=True)
    with open("16dreamer_special.txt", "rt") as f:
        prompt_q8_4_special = f.read()
    model_round = {"mistral-large-latest": [(i, i + 9) for i in range(1, 151, 9 + 1)]}

    with concurrent.futures.ProcessPoolExecutor(max_workers=28) as executor:
        futures_q8_4_special = [
            executor.submit(exp, exp_amb_dir, prompt_q8_4_special, exp_model_name, start, end, 2000)
            for exp_model_name, ranges in model_round.items()
            for start, end in ranges
        ]